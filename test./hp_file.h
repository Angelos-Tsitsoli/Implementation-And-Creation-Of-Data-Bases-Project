...
//#define Max_Recods (BF_BLOCK_SIZE - / (sizeof(record)))

/* Η δομή HP_info κρατάει μεταδεδομένα που σχετίζονται με το αρχείο σωρού*/
typedef struct {
    int fileDesc;
    int id; //0 -> heap file, 1 -> hash table
    int NumRecords; //Max number of records which can stored in a block
    int IdLast; //Id of the last record of the heap file
} HP_info;

typedef struct 
{
    int NumRecords; //Number of records in this block
